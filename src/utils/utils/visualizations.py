...
def plot_narrative_distribution(df: pd.DataFrame, colors: Dict[str, str], 
    ...
    for bar, value in zip(bars, narrative_dist.values):
        ax2.text(bar.get_x() + bar.get_width()/2,
            bar.get_height() + max(narrative_dist.values)*0.01,
...
def plot_market_cap_analysis(df: pd.DataFrame, colors: Dict[str, str], 
    ...
    # Gráfico de barras
    plot_colors = [colors.get(
        n,
        f'C{i}') for i,
        n in enumerate(market_cap_by_narrative.index
    )]
    bars = ax1.bar(
        market_cap_by_narrative.index,
        market_cap_by_narrative.values/1e9,
        color=plot_colors
    )
...
def plot_temporal_analysis(df: pd.DataFrame, colors: Dict[str, str], 
    ...
    if 'market_cap' in df_temp.columns:
        ...
        pivot_temporal = temporal_data.pivot(
            index='date',
            columns='narrative',
            values='market_cap').fillna(0
        )
    # --- Sección de validación y resultados ---
    ...
    pivot_tokens = active_tokens.pivot(
        index='date',
        columns='narrative',
        values='id').fillna(0
    )
    ...
    activity_heatmap = df_temp.groupby(['day_of_week',
        'narrative']).size().unstack(fill_value=0)
...
def plot_returns_analysis(df: pd.DataFrame, colors: Dict[str, str], 
    ...
    if not return_cols:
        fig.text(
            0.5,
            0.5,
            'Datos de retornos no disponibles',
            ha='center',
            va='center',
            fontsize=20
        )
